...
export async function getIssuerOverviewAction(organizationId: string): Promise<IssuerOverview | ActionFailure> {
...
export async function startDomainVerificationAction(
  ...
  domain: string
): Promise<{ recordName: string; recordValue: string } | ActionFailure> {
  return guarded(async () => {
    await requireIssuerOrg(organizationId)

    const normalized = domain.trim().toLowerCase().replace(/^https?:\/\//, '').replace(/\/.*$/, '')
    if (!/^[a-z0-9.-]+\.[a-z]{2,}$/.test(normalized)) {
      throw new UserFacingError('Enter a valid domain, e.g. university.edu')
    }
...
export async function checkDomainVerificationAction(
  organizationId: string
): Promise<{ verified: boolean; message: string } | ActionFailure> {
  return guarded(async () => {
    const organization = await requireIssuerOrg(organizationId)
...
export async function ensureIssuerKeyAction(organizationId: string): Promise<IssuerPublicKey | ActionFailure> {
  return guarded(async () => {
    await requireIssuerOrg(organizationId)
    await getOrCreateActiveIssuerKey(organizationId)
    const publicKey = await getIssuerPublicKey(organizationId)
...
export async function getActiveIssuerKeyAction(organizationId: string): Promise<IssuerPublicKey | null | ActionFailure> {
  return guarded(async () => {
    await requireIssuerOrg(organizationId)
