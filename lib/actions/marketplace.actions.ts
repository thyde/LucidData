...
export async function getOpenPoolsAction(category?: string): Promise<OpenDataPool[] | ActionFailure> {
  return guarded(async () => {
    await getAuthenticatedUserId()
    return listOpenPools(category)  })
}

export async function getOpenPoolAction(id: string): Promise<DataPool | null | ActionFailure> {
  return guarded(async () => {
    await getAuthenticatedUserId()
    return getOpenPool(id)  })
}

export async function getOrgPoolsAction(orgId: string): Promise<DataPool[] | ActionFailure> {
  return guarded(async () => {
    await requireOrgMembership(orgId)
    return listOrgPools(orgId)  })
}

export async function getMarketSupplyAction(orgId: string): Promise<MarketSupplyRow[] | ActionFailure> {
  return guarded(async () => {
    await requireVerifiedDataBuyer(orgId)
    return getMarketSupply()  })
}

export async function createPoolAction(orgId: string, input: unknown): Promise<DataPool | ActionFailure> {
  return guarded(async () => {
...
export async function closePoolAction(orgId: string, poolId: string): Promise<DataPool | ActionFailure> {
  return guarded(async () => {
    const userId = await getAuthenticatedUserId()
...
export async function evaluatePoolAction(
  orgId: string,
  poolId: string
): Promise<PoolEvaluation | ActionFailure> {
  return guarded(async () => {
    await requireDataBuyer(orgId)
